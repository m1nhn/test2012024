...
int do_action(int sock, int opt)
{
	...
	case 1:
	{
		...
		{
			sprintf(command2,
                "echo \"Subject: Flight Booking Confirmation\n\nDear our dearest customer,\n\nWe are delighted to inform you that your flight booking has been confirmed. Please check on the system for the details of your booking:\n\nmail: %s\nairplane ID: %d\nnumber of seats: %d\n\nThank you for choosing our services. Have a pleasant journey!\n\nBest regards,\nYour Airline Team\" > email.txt",
                mail, airplaneid, required_seats);
			system(command2);
			sprintf(command,
					"curl -s --url 'smtp://smtp.gmail.com:587' --ssl-reqd "
					"--mail-from 'toniminh161200@gmail.com' --mail-rcpt %s "
					"--user 'toniminh161200@gmail.com:tkxj thcp fpzf exna' --tlsv1.2 --upload-file email.txt",
					mail);
			system(command);
			if (system(command) == 0)
			{
